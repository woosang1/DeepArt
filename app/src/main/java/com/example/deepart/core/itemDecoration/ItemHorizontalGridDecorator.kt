...
class ItemHorizontalGridDecorator(
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        outRect.run {
            top = if (layoutParams.spanIndex == 0) topMargin else (middleVerticalMargin / 2)
            bottom = if (layoutParams.spanIndex == (spanCount - 1)) bottomMargin else (middleVerticalMargin / 2)
            left = if (line == 1) startMargin else (middleHorizontalMargin / 2)
            right = if (line == maxLine) endMargin else (middleHorizontalMargin / 2)
